...
func New(scheme *goa.JWTSecurity, keystore interface{}, options ...Option) goa.Middleware {
	...
	oo.Authentication = Authentication
	oo.Authorization = Authorization
	...
	return func(nextHandler goa.Handler) goa.Handler {
		return func(ctx context.Context, rw http.ResponseWriter, req *http.Request) error {
			token, err := parseToken(oo.Scheme, oo.Keystore, req)
			if err != nil {
				return err
			}
			...
			if oo.Authentication != nil {
				_, err = oo.Authentication(ctx, claims)
				if err != nil {
					return err
				}
			}
